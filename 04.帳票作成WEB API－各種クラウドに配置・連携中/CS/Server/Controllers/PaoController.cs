...
namespace Server.Windows.Controllers
#else
using Pao.Reports.Linux;
namespace Server.Linux.Controllers
#endif
{
    ...
    public class PaoController : ControllerBase
    {
        ...
        public PrintData Get(int Id)
        {
            ...
            switch (Id)
            {
                ...
                default:
                    ...
                    //帳票定義体の読み込み
                    string path = Path.Combine(Directory.GetCurrentDirectory(), @"App_Data/simple.prepd");
                    ...
                    //string text = "Azure Linuxで\n\r作った印刷データですよ～ん♪";
                    //string text = "AWS EC2 - Amazon Linux 2023\n\rで作った印刷データですよ～ん♪";
                    //string text = "GCP(GCE) Debian GNU/Linux\n\rで作った印刷データですよ～ん♪";
                    text += "\n\rREST API で変更しましたよ！！ \n\r(GET)";
        ...
        [HttpPost]
        public PrintData Post(PrintData pd)
        {
            #region 接続文字列
            sqlcon = new SqlConnection("Server=tcp:fzxu46e9ck.database.windows.net,1433;Initial Catalog=Reports.net.Sample;Persist Security Info=False;User ID=AzureLab;Password=ayakaRk9504w;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;");
            #endregion
        ...
        {
            //帳票定義体の読み込み
            string path = Path.Combine(Directory.GetCurrentDirectory(), @"App_Data/sample10.prepd");
            paoRep.LoadDefFile(path);
        ...
        protected string MakeReports郵便番号(IReport paoRep)
        {
            ...
            // デザインファイル
            string path1 = Path.Combine(Directory.GetCurrentDirectory(), @"App_Data/zipcode1.prepd");
            string path2 = Path.Combine(Directory.GetCurrentDirectory(), @"App_Data/zipcode2.prepd");
        ...
        protected string MakeReports請求書(IReport paoRep)
        {
            ...
            foreach (DataRow hdr in ht.Rows)
            {
                ...
                for (; ii < dv.Count; ii++)
                {
                    ...
                    paoRep.Write("field2", gpath, ii + 2);
        ...
        protected string MakeReports商品一覧(IReport paoRep)
        {
            ...
            int cnt小分類 = 0;
#if NET8_0_OR_GREATER
            List<PrintData商品> pds = [];
#else
            List<PrintData商品> pds = new();
#endif
            PrintData商品 pd;
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (sv大分類名称 != (string)dr["大分類名称"])
                {
                    pd.s大分類名称 = (string)dr["大分類名称"];
                }
                if (sv小分類名称 != (string)dr["小分類名称"])
                {
                    pd.s小分類名称 = (string)dr["小分類名称"];
                }
