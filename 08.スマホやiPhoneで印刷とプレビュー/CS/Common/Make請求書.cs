using Pao.Reports;
using Color = Pao.Reports.Linux.Color;
...
namespace SvgPreviewMobile
{
    static class Make請求書
    {
        public static void SetupData(IReport paoRep)
        {
            ...
            ds.Tables.Add(Util.ExecuteQuery("select * from 請求ヘッダ ORDER BY 請求番号", connection));
            ds.Tables[0].TableName = "請求ヘッダ";

            ds.Tables.Add(Util.ExecuteQuery("select * from 請求明細 ORDER BY 請求番号, 行番号", connection));
            ds.Tables[1].TableName = "請求明細";

            paoRep.LoadDefFile(Util.SharePath + "invoice.prepd");

            // 会社角印画像ファイルパス
            string gpath = Util.SharePath + "stamp.png";
